...
-- 3.1) write all details where last_name should be unique 

select * from actor where last_name in(select distinct last_name from actor);
...
insert into rental (rental_date,inventory_id,customer_id,return_date,staff_id) values(date(now()),1,1,date_add(now(),interval 6 day),1);   #checkout the rental duration for finding the 6 days
...
# answer :- NATURAL JOIN joins the two table based on same attributes and datatypes (this is not recomanded because it gives unexpected output)  it only used where you want to join the 2 table with same column  

# answer :- INNER JOIN joins the two table on the basis of the column which is explicitly specified in the on clause or by using clause
