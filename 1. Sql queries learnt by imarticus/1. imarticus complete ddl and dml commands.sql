...
CREATE TABLE information (
	...
	City varchar(20));                 # Where not null is not defined so by default it is null (if you mention null or you can leave it empty)
...
## using auto increment constraints , using the default constraints
drop table if exists personal_info;
create table personal_info(
...
truncate table personal_info;                   # truncate is used to clear the record present in the table and to keep the structure(column name) as it is
...
describe order_info;               # it tells you the cust_id is a foreign key or not

select * from personal_info;
insert into order_info values (1, 1, 1);
insert into order_info values (2, 1, 3);
insert into order_info values (3,3,5);         #it shows an error because the cust_id is the foreign key in order_info table with references of personal_info table, so always we want to check it out  the data we are providing in foreign key column should be avaliable in the personal_info table(referencing column)
select * from order_info;
select * from personal_info;


## Will be used to drop the entire table with contents from the database.
## It will delete the structure as well.
...
alter table order_info modify orderid smallint;            #if we want to change the data type, constraints then we use alter and modify command
describe order_info;                                       # we use describe check the data type,null values,key of the column(attributes)
describe personal_info;

alter table personal_info drop PRIMARY KEY;               # if you want to drop the primary key

alter table order_info add column quantity int not null;             # if you want to add the column into the existing table
...
alter table personal_info add column gender char(1) first;           # if you want to add the column at the first
select * from order_info;
select * from personal_info;
describe personal_info;
alter table order_info add constraint fk_cust_id
foreign key (cust_id) references personal_info (cust_id);
describe order_info;
...
select * from personal_info;
update personal_info set gender ='m' ;
update personal_info set city ='mysore' where city is null;
...
create table pfbackup  select * from personal_info;          # it create the table like personal_info and all the record and datatypes will be copied 
