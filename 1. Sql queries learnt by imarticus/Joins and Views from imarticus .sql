...
## Inner Join           # it only prints the records which are present in both the table(equi join)
select 
	c.cust_id,
    c.cust_name,
    c.contact_name,
...
select * from orders;
## Right Join              # it prints every record from the right table and intersected records(right outer join)
...
## Left Join          # it prints every record from left table and intersected record in left table  (left outer join)
...
## Full Join         # it prints everything which are present in left table +wich are present in right table
select 
	c.cust_id,
    c.cust_name,
    c.contact_name,
...
/* why view is important ?
ans:- view is called as virtual tables, you can use view to hide table columns from users by granting them access to the view and not to the table itself 
This helps to enhance database security and integrity*/
...
## Join View         # it joins the two table or table itself
select * from employee;
...
left join employee mgr on
 ...
 create or replace view manager as select e.emp_id,e.emp_name,e.emp_desig,m.emp_name as manager from employee e left join employee m on e.rep_manager =m.emp_id;
...
/* sub query
A subquery is nested into another sql query and embadded with select,insert,update or delete statement along with various operator 
The inner query executed first and gives the result to the outer query  and then main outer query will be performed*/
