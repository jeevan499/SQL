...
select department from worker group by department;    # it does the same work
...
select *,count(salary) from worker group by salary having count(salary)>1;
select *,count(salary) from (select * from worker group by salary having count(salary)>1);
...
select department,first_name,max(salary) higest_salary from worker group by department;

# Q-46. Write An SQL Query To Fetch Three Max Salaries From A Table.
select * from worker group by salary order by salary desc limit 3;

# Q-47. Write An SQL Query To Fetch Three Min Salaries From A Table.
select * from worker group by salary order by salary limit 3;

# Q-48. Write An SQL Query To Fetch Nth Max Salaries From A Table. say 5
select * from worker group by salary order by salary desc limit 4,1;
...
select concat(first_name,' ',last_name) from worker where salary in (select * from (select salary from worker group by salary order by salary desc limit 1)as t);
